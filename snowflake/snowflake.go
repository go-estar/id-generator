...
import (
	...
	idGenerator "github.com/go-estar/id-generator"
	"regexp"
	"strconv"
	"sync"
	"time"
...
func New(n int64, conf ...*NodeConfig) *idGenerator.Generator {
...
func NewWithConfig(c *config.Config, nc ...*NodeConfig) *idGenerator.Generator {
	node := getNodeWithConfig(c)
	return New(node, nc...)
}

func NewWithConfig15(c *config.Config) *idGenerator.Generator {
	node := getNodeWithConfig(c)
...
func NewWithConfig16(c *config.Config) *idGenerator.Generator {
	node := getNodeWithConfig(c)
	return New(node, &NodeConfig{
		Epoch:    1577808000000,
		NodeBits: 4,
...
func NewWithConfig19(c *config.Config) *idGenerator.Generator {
	node := getNodeWithConfig(c)
	return New(node, &NodeConfig{
		Epoch:    1577808000000,
		NodeBits: 10,
